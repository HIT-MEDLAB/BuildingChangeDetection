import {
  ...
  describe("Results component", () => {
    ...
    test(
      "shows loading state while results are being fetched",
      () => {
    ...
    test(
      "shows an error when loading the inspection fails",
      async () => {
    ...
    test(
      "navigates back to Upload after a loading error",
      async () => {
    ...
    test(
      "shows a processing message when inspection is not completed",
      async () => {
    ...
    test(
      "shows a failure message when analysis failed",
      async () => {
        api.get.mockResolvedValue({
          data: {
            id: "123",
    ...
    test(
      "shows the number of detected changes",
      async () => {
        api.get.mockResolvedValue({
    ...
    test(
      "handles missing bounding boxes safely",
      async () => {
        api.get.mockResolvedValue({
          data: {
            id: "123",
    ...
    test(
      "renders every detected bounding box",
      async () => {
        api.get.mockResolvedValue({
          data: completedInspection,
        });
    ...
    test(
      "shows an error when case status update fails",
      async () => {
        const user = userEvent.setup();
    ...
    test(
      "shows an error when the report is unavailable",
      async () => {
        const user = userEvent.setup();
